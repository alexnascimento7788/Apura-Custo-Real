public class Mercadoria {
    ...
    private double getAliqIcmsTraduzida() { return (aliqIcms == 0) ? 0 : aliqIcms / 10000.0; }
    private double getFatorIpiTratado() { return aliqIpi / 100.0; }
    ...
    public double getIpi() {
        double fator = getFatorIpiTratado();
    ...
    public double getPis() {
        ...
        return Math.round((baseCalculo * 0.0065) * 100.0) / 100.0;
    }

    public double getCofins() {
        double fatorIcms = getAliqIcmsTraduzida();
        double baseCalculo = pVendaTakken - (pVendaTakken * fatorIcms);
        return Math.round((baseCalculo * (aliqCofinsSai / 100.0)) * 100.0) / 100.0;
    }

    public double getIrppj() { return Math.round((pVendaTakken * (irppj / 100.0)) * 100.0) / 100.0; }
    public double getCsll() { return Math.round((pVendaTakken * (csll / 100.0)) * 100.0) / 100.0; }
    ...
    public double getLucroLiquido() { return Math.round((pVendaTakken - getCustoRealFinal()) * 100.0) / 100.0; }

    public double getMarkdown() {
        double lucro = getLucroLiquido();
        if (lucro <= 0 || pVendaTakken == 0) return 0.0;
        return Math.round(((lucro / pVendaTakken) * 100.0) * 100.0) / 100.0;
    }
}